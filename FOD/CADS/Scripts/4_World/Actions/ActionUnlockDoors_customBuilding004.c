...
class ActionUnlockDoors_customBuilding004: ActionContinuousBase
{
	void ActionUnlockDoors_customBuilding004()
	{
	...
	//	Print(target.GetObject().ToString());
		if(target.GetObject().ToString().Contains("dayz_house_test4"))
		{
			// do nothing
		}
		else
